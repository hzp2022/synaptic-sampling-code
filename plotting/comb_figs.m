...
% 将不同figures图片合并到一个figure中。
% 
% 调用格式：  
%    CombFigs():                  不带任何输入参数，将当前目录下所有fig文件合并成一个fig文件。默认合并后的fig文件名为"合并Figure文件.fig"。
%    CombFigs(figname):           figname为合并后的fig文件名，将当前目录下所有fig文件合并为figname文件。
%    CombFigs(figname，figfiles): figname为合并后的fig文件名,figfiles为待合并的fig文件，cell数组格式，将figfiles文件合并为figname文件。 
% 
